...
public class ConfigContainer implements Constants {
	...
	public static ConfigContainer getConfigContainer(String fileName) throws IOException {
		if (c == null) {
			c = new ConfigContainer();
			File f = new File(fileName);
			if (!f.exists() || f.isDirectory()) {
				System.out.println("Given file is a directory or does not exist");
				return null;
			}
			c.extension = fileName.substring(fileName.lastIndexOf(".") + 1, fileName.length());
